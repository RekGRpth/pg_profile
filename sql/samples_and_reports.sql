...
SELECT gs.id, gs.id FROM generate_series(1,1000) as gs(id);
INSERT INTO profile.test_rel_storage_params2(id, val)
SELECT gs.id, gs.id FROM generate_series(1,1000) as gs(id);
UPDATE profile.test_rel_storage_params1 SET val = val + 1 WHERE id % 2 = 0;
UPDATE profile.test_rel_storage_params2 SET val = val + 1 WHERE id % 2 = 1;
SELECT count(*) FROM profile.test_rel_storage_params1 WHERE id < 100;
SELECT count(*) FROM profile.test_rel_storage_params2 WHERE id < 100;
